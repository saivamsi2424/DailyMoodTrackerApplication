...
class App extends Component {
  ...
  changeMonthForReport = event => {
    this.setState({activeMonthForMainReport: event.target.value})
  }
  ...
  updateEmojiCount = () => {
    ...
    monthsList.forEach(eachMonth => {
      eachMonth.dates.forEach(eachItem => {
        if (eachItem.emojiName === 'Very Happy') {
          veryHappyCount += 1
        } else if (eachItem.emojiName === 'Happy') {
          happyCount += 1
        } else if (eachItem.emojiName === 'Neutral') {
          neutralCount += 1
        } else if (eachItem.emojiName === 'Sad') {
          sadCount += 1
        } else if (eachItem.emojiName === 'Very Sad') {
          verySadCount += 1
        }
      })
    })
    const newList = emojisList.map(item => {
      let count
    ...
    this.setState({emojisList: newList})
    console.log(veryHappyCount)
  }
  ...
  updateReportValue = () => {
    ...
    const currentMonth = monthsList[activeMonthForCalender - 1].dates
    const monthLength = currentMonth.length
    let dates = []

    if (dayNameForMiniReport === 'Sun' && monthLength >= 29) {
      dates = [1, 8, 15, 22, 29]
    } else if (dayNameForMiniReport === 'Mon') {
      dates = monthLength >= 30 ? [2, 9, 16, 23, 30] : [2, 9, 16, 23]
    } else if (dayNameForMiniReport === 'Tue') {
      dates = monthLength >= 31 ? [3, 10, 17, 24, 31] : [3, 10, 17, 24]
    } else if (dayNameForMiniReport === 'Wed') {
      dates = [4, 11, 18, 25]
    } else if (dayNameForMiniReport === 'Thu') {
      dates = [5, 12, 19, 26]
    } else if (dayNameForMiniReport === 'Fri') {
      dates = [6, 13, 20, 27]
    } else if (dayNameForMiniReport === 'Sat') {
      dates = [7, 14, 21, 28]
    }

    const count = currentMonth.filter(dateObj => {
      const dateNumber = parseInt(dateObj.date)
      return (
        dates.includes(dateNumber) &&
        dateObj.emojiName === emojiNameForMiniReport
      )
    }).length

    this.setState({reportValue: count})
  }

  changeTheEmojiNameForMiniReport = event => {
    this.setState(
      {emojiNameForMiniReport: event.target.value},
      this.updateReportValue,
    )
  }

  changeTheDayNameForMiniReport = event => {
    this.setState(
      {dayNameForMiniReport: event.target.value},
      this.updateReportValue,
    )
  }
  ...
  decrementMonth = () => {
    this.setState(
      prevState => ({
        activeMonthForCalender:
          prevState.activeMonthForCalender > 1
            ? prevState.activeMonthForCalender - 1
            : 12,
      }),
      this.updateReportValue,
  ...
  incrementMonth = () => {
    this.setState(
      prevState => ({
        activeMonthForCalender:
          prevState.activeMonthForCalender < 12
            ? prevState.activeMonthForCalender + 1
            : 1,
      }),
      this.updateReportValue,
  ...
  updateSelectedActiveEmojiId = id => {
    this.setState({selectedActiveEmojiId: id})
  }

  updateEmojiForDate = id => {
    const {
    ...
    const emojiData = emojisList.find(each => each.id === selectedActiveEmojiId)
    if (!emojiData) return

    const updatedMonthsList = monthsList.map((month, index) => {
      if (index === activeMonthForCalender - 1) {
        const updatedDates = month.dates.map(date => {
          if (date.id === id) {
            if (date.emojiUrl === '') {
              return {
                ...date,
            ...
            if (date.emojiUrl === emojiData.emojiUrl) {
              return {
                ...date,
                emojiUrl: '',
                emojiName: '',
        ...
        return {...month, dates: updatedDates}
      }
      return month
    })
    ...
    return (
      <Context.Provider
        value={{
          ...
          emojisList,
          selectedActiveEmojiId,
          updateSelectedActiveEmojiId: this.updateSelectedActiveEmojiId,
          updateEmojiForDate: this.updateEmojiForDate,
          decrementMonth: this.decrementMonth,
          incrementMonth: this.incrementMonth,
          dayNameForMiniReport,
          emojiNameForMiniReport,
          changeTheEmojiNameForMiniReport: this.changeTheEmojiNameForMiniReport,
          changeTheDayNameForMiniReport: this.changeTheDayNameForMiniReport,
          reportValue,
          activeMonthForMainReport,
          changeMonthForReport: this.changeMonthForReport,
        }}
      >
        <Switch>
